...
class GroupChannelChatViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, UIImagePickerControllerDelegate, UINavigationControllerDelegate, RSKImageCropViewControllerDelegate, GroupChannelMessageTableViewCellDelegate, GroupChannelSettingsDelegate, UIDocumentPickerDelegate, NotificationDelegate, SBDNetworkDelegate, SBDConnectionDelegate, SBDChannelDelegate, SBDMessageCollectionDelegate {
    ...
    override func viewDidLoad() {
        ...
        self.startingPoint = Int64.max
        createMessageCollection(channel: channel!, startingPoint: self.startingPoint!)
    }
    ...
    private func getIndexOfMessage(timestamp: Int64) -> Int {
        let datasourceIsEmpty = -2
        let messageDoesNotExist = -1
        ...
        if let targetIndex = self.messages.index(where: { $0.createdAt == timestamp }) {
            ...
            return messageDoesNotExist
        }
    }
    
    // if returns -1 => append at the end.
    // if not -1 => append in the middle.
    private func getIndexToInsert(message: SBDBaseMessage) -> Int {
        if self.messages.isEmpty { return -1 }
        
        for msg in self.messages {
            if msg.createdAt > message.createdAt {
                // append in the middle.
                let index = self.messages.firstIndex(of: msg)!
                return index
            }
            
        }
        // append at the end of self.messages
        return -1
    }
    
    func updateTableViewOnLoadNext(messages: [SBDBaseMessage]) {
    ...
    func updateTableViewOnLoadPrevious(messages: [SBDBaseMessage], initial: Bool) {
    ...
    func loadNextMessages(initial: Bool) {
        ...
        if messageCollection!.hasNext {
            self.messageCollection!.loadNext { messages, error in
                ...
                self.updateTableViewOnLoadNext(messages: messages)
            }
        } else {
            self.isLoading = false
        }
    ...
    func loadPreviousMessages(initial: Bool) {
        ...
        if messageCollection!.hasPrevious {
            self.messageCollection!.loadPrevious(completionHandler: { messages, error in
                if error != nil {
                    self.isLoading = false
                ...
                // update datasource & tableview.
                self.updateTableViewOnLoadPrevious(messages: messages, initial: initial)
            })
        }
    }
    ...
    @IBAction func clickSendUserMessageButton(_ sender: Any) {
        ...
        preSendMessage = channel.sendUserMessage(messageText) { (userMessage, error) in
            /// do something with sent message
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if indexPath.row == 0 {
            if self.messages.count > 0 && self.initialLoading == false && self.isLoading == false {
    ...
    func scrollViewDidScroll(_ scrollView: UIScrollView) {
        if scrollView == self.messageTableView {
            ...
            if offsetY > contentHeight - scrollView.frame.height - 60
            {
                self.loadNextMessages(initial: false)
            }
        }
    }
    ...
    func messageCollection(_ collection: SBDMessageCollection, context: SBDMessageContext, channel: SBDGroupChannel, addedMessages messages: [SBDBaseMessage]) {
        ...
        case .succeeded:
                for message in messages {
                    ...
                    let index = self.getIndexToInsert(message: message)
    ...
    func messageCollection(_ collection: SBDMessageCollection, context: SBDMessageContext, channel: SBDGroupChannel, updatedMessages messages: [SBDBaseMessage]) {
        ...
        case .succeeded:
            ...
            if context.source == .eventMessageSent {
                // Case where pending messages state is updated to success.
                for message in messages {
        ...
        case .failed:
            for message in messages {
                ...
                DispatchQueue.main.async {
                    ...
                    // Add the failed message to resendableMessages.
                    if self.resendableMessages[requestId] != nil {
                        self.resendableMessages.removeValue(forKey: requestId)
                    }
                }
        ...
        DispatchQueue.main.async {
            for msg in messages {
                if msg.sendingStatus == .succeeded {
                    self.deleteMessageFromTableView(msg.messageId)
                } else if msg.sendingStatus == .failed || msg.sendingStatus == .pending {
                    self.deleteMessageFromTableView(msg.requestId)
                }
            }
        }
    }
    ...
    func messageCollection(_ collection: SBDMessageCollection, context: SBDMessageContext, updatedChannel channel: SBDGroupChannel) {
        ...
        if source == .eventTypingStatusUpdated {
            let typingIndicatorText = Utils.buildTypingIndicatorLabel(channel: channel)
        ...
        } else if source == .eventReadReceiptUpdated {
        ...
        } else if source == .eventChannelChanged {
            DispatchQueue.main.async {
    ...
    func didDetectHugeGap(_ collection: SBDMessageCollection) {
        var defaultStartingPoint =  Int64.max
        if self.messageCollection != nil {
            defaultStartingPoint = self.messageCollection!.startingPoint
            self.messageCollection!.dispose()
        }
        
        // get first item's position
        let firstIndexPath = messageTableView.indexPathsForVisibleRows?.last
        if firstIndexPath != nil {
            let message = self.messages[firstIndexPath!.row]
            createMessageCollection(channel: channel!, startingPoint: message.createdAt)
        } else {
            createMessageCollection(channel: channel!, startingPoint: defaultStartingPoint)
        }
    }
    
    private func createMessageCollection(channel: SBDGroupChannel, startingPoint: Int64) {
        let params = SBDMessageListParams()
        params.reverse = false
        params.nextResultSize = 30
        params.previousResultSize = 30
        params.isInclusive = true
        
        // Dispose previously existing messageCollection.
        if self.messageCollection != nil {
            self.messageCollection!.dispose()
        }
        
        self.messageCollection = SBDMessageCollection.init(channel: channel, startingPoint: startingPoint, params: params)
        self.messageCollection?.delegate = self
        
        self.messageCollection?.start(with: .cacheAndReplaceByApi, cacheResultHandler: { messages, error in
            ...
            // update
            guard let messages = messages else { return }
            self.updateTableViewOnLoadPrevious(messages: messages, initial: true )
            ...
            guard let messages = messages else { return }
            self.messages = [SBDBaseMessage]()  // clear message datasource.
            self.updateTableViewOnLoadPrevious(messages: messages, initial: true )
        })
    }
    ...
    private func deleteMessageFromTableView(_ requestId: String) {
        if self.messages.count == 0 {
            return
        }
